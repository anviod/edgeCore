...
layout: default
title: 边缘计算基础功能
description: edgeCore 边缘计算基础功能 — 规则引擎与数据流转说明
---

# 边缘计算基础功能

> **文档定位：** 规则引擎与数据流转说明。介绍 edgeCore 边缘计算的当前架构、数据闭环与规则执行能力。规则配置细节见 [边缘计算规则帮助](边缘计算规则帮助.html)；API 见 [边缘计算 API](../API/Edge_Computing_CN.html)。
...
edgeCore 边缘计算在网关上提供**事件驱动的本地规则引擎**，与南向采集、影子设备、北向推送共用同一数据快照，实现：
