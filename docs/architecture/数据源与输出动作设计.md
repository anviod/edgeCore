...
layout: default
title: 数据源与输出动作设计说明
description: edgeCore 数据源与输出动作设计（收敛架构版）
---
...
> **PRIMARY 架构目标（2026-07 收敛）**：edgeCore 工业边缘的**默认正确路径**是 **V2.2 Pipeline Worker**（ARMv7 / RK3588 单线程内联 Pipeline）。本文 §1.2–§1.3 描述的 Snapshot → Compute Executor → Action Runtime 链路对应 **Gateway Rich Profile（V2.1 可选）**；嵌入式部署请参阅 **§1.0 嵌入式 Profile** 与 [边缘计算优化升级 2.0](../TODO/边缘计算优化升级2.0.md)。
...
| **禁止** | 运行时 YAML | 禁止为业务配置引入 YAML 解析；禁止配置文件热重载（与现有 `edgeCore-db-runtime-architecture` 一致） |
