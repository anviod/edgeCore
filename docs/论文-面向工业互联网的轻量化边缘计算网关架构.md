...
edgeCore 在 EAN 2.0 中的角色是 **Capability Runtime**（能力注册、发现发布、Invoke 执行、Event 上报）；EdgeOS 的角色是 **Coordination Platform**（全局发现索引、跨节点编排、Invoke 发起、Event 订阅与规则）。两者通过 MQTT v5 / NATS 2.x 互联，Topic/Subject 统一使用 `$edgeos/...` 字符串形式。
