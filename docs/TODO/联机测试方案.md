...
│ 启动: go run cmd/main.go │                           │ 启动: systemctl edgeCore       │
...
go run -ldflags="-X github.com/anviod/edgeCore/internal/model.Version=0.0.4 \
                 -X 'github.com/anviod/edgeCore/internal/model.BuildTime=$(date -u +%Y-%m-%dT%H:%M:%SZ)' \
                 -X github.com/anviod/edgeCore/internal/model.CommitID=$(git rev-parse --short HEAD)" \
       cmd/main.go
```
...
步骤 6: 打包 .deb                 # dpkg 包（含 preinstall/postinstall/edgeCore.service）
...
```
dist/
├── edgeCore-v0.0.4-amd64.deb          # AMD64 deb 安装包
├── edgeCore-v0.0.4-arm64.deb          # ARM64 deb 安装包（远程节点使用）
├── edgeCore-v0.0.4-arm.deb            # ARMv7 deb 安装包
├── edgeCore-0.0.4-linux-amd64.tar.gz  # AMD64 归档
├── edgeCore-0.0.4-linux-arm64.tar.gz  # ARM64 归档
├── edgeCore-0.0.4-linux-arm.tar.gz    # ARMv7 归档
├── edgeCore-0.0.4-windows-amd64.tar.gz
├── edgeCore-0.0.4-windows-arm64.tar.gz
└── SHA256SUMS
```
...
tar -xzf dist/edgeCore-0.0.4-linux-amd64.tar.gz -C /tmp/edgeCore-test
/tmp/edgeCore-test/edgeCore --version 2>/dev/null || \
  strings /tmp/edgeCore-test/edgeCore | grep -E "^(0\.0\.[0-9]+|$(git rev-parse --short HEAD))"
```
...
[步骤 3] 查找匹配的 .deb 包
    ├── ls dist/edgeCore-v*-arm64.deb | sort -V | tail -1
    └── 未找到 → 终止部署，提示先执行 goreleaser 构建

[步骤 4] SCP 传输安装包
    └── scp dist/edgeCore-v0.0.4-arm64.deb root@192.168.3.230:/tmp/

[步骤 5] 备份现有配置
    ├── cp -rf /usr/local/bin/edgeCore/data → /tmp/edgeCore_backup/
    └── cp -rf /usr/local/bin/edgeCore/config → /tmp/edgeCore_backup/

[步骤 6] 停止旧服务
    └── systemctl stop edgeCore

[步骤 7] 卸载旧版本
    └── apt remove -y edgeCore

[步骤 8] 安装新版本
    ├── dpkg -i /tmp/edgeCore-v0.0.4-arm64.deb
    └── 依赖缺失 → apt-get install -f -y 自动修复

[步骤 9] 配置节点名称
    └── sed -i "s/node_name:.*/node_name: NODE-REMOTE/" /usr/local/bin/edgeCore/config/sync.yaml
...
[步骤 11] 验证服务状态
    └── sleep 5 && systemctl is-active edgeCore → active
```
...
ssh root@192.168.3.230 "journalctl -u edgeCore -n 50 --no-pager"
```
...
apt remove -y edgeCore 2>/dev/null || true

# 5. 安装
dpkg -i /tmp/edgeCore-v0.0.4-arm64.deb || apt-get install -f -y

# 6. 配置节点名
sed -i 's/node_name:.*/node_name: NODE-REMOTE/' /usr/local/bin/edgeCore/config/sync.yaml

# 7. 启用并启动
systemctl daemon-reload && systemctl enable edgeCore && systemctl start edgeCore
...
| **目标** | 验证节点离线被正确检测并标记 |
| **前置条件** | 双节点在线 |
| **测试步骤** | 1. 停止 NODE-REMOTE（`systemctl stop edgeCore`）<br>2. 等待心跳超时（30s） |
...
| E-03 | 远程服务崩溃 | `kill -9` edgeCore 进程 | systemd 自动重启（Restart=always）；恢复后增量同步 |
...
# 实时跟踪
ssh root@192.168.3.230 "journalctl -u edgeCore -f"
