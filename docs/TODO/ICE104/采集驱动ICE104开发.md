...
| `Write`     | 写入模式，支持edgeCore主动写入数据到设备 | 遥控遥调操作    |
...
| 数据采集测试 | 修改模拟器点位值    | edgeCore 收到变化数据     |
...
- `edgeCore_server.c` — 最小 CS104 从站，响应总召唤返回 M_ME_NA_1 IOA=1
...
```bash
./edgeCore_server --bind 127.0.0.1 --port 2404
CGO_ENABLED=0 go test -tags=integration ./internal/driver/ice104/... \
...
#### 11.2.5 edgeCore 点位配置（与 §11.1 相同）
